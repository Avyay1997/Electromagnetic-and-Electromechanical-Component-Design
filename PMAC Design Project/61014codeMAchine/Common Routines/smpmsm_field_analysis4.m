%  FUNCTION:    [qr,Btqr,pBtqr,Bbqr,pBbqr, ...
%               Bt_max,Bb_max,Btb_max,Brot_max,Hm_min]= ...
%               smpmsm_field_analysis4(Nrp,P,ns,l,rr,ri,rm,wt,db, ...
%                                        rf,np,dm,alphapm,bpm, ...
%                                        mur_pm,mur_sp,g,is,phii, ...
%                                        phism_te,phism_tc)
...
%                          (rad)
%
% OUTPUTS:      qr       - electrical rotor positions (rad)
...
%                          respect to electrical rotor position 
%                          at corresponding rotor positions (T/rad) 
%               Brbt_mx  - Maximum tang. flux density on rotor backiron(T)
%               Brbr_mx  - Maximum rad. flux density in rotor backiron (T)
...
%               Phit     - number of rotor positions by number of slots
...
%
%               Modified on 12/2/2012 in order to select better rotor
%               position points.

function [qr,Btqr,pBtqr,Bbqr,pBbqr, ...
          Brbt_mx,Brbr_mx,Hm_min]= ...
          smpmsm_field_analysis4(Nrp,P,ns,l,rr,ri,rm,wt,db, ...
                                 rf,np,dm,alphapm,bpm, ...
                                 mur_pm,mur_sp,g, ...
                                 is,phii,phism_tc)
                             
% compute location of tooth edges
phism_te=phism_tc-pi/ns;
...
if Nrp>1
  qrm=(2*pi/ns)*linspace(0,(Nrp-1)/Nrp,Nrp);            
else
  qrm=0.0;
end
...
%       offset=phism_te(1);
%       phis_te=mod((phism_te-offset)*P/2,2*pi);
%       phis_mt=mod((phism_mt-offset)*P/2,2*pi);
%       phism_tp=sort([phis_mt phis_te])*2/P+offset;
...
Brot_max1=max(abs([B1 B2 B3 B4 B5]));

% Compute the rotor flux density manitude based on sectors aligned
% with stator teeth
Brot_rad=Phit*ns/(2*pi*rr*l);
Brbr_mx=max(max(max(abs(Brot_rad))),Brot_max1);
Brot_tan=Phib/(l*(rr-ri));
Brbt_mx=max(max(abs(Brot_tan)));

% Computer tooth 1 fields
[pBtqr,Btqr,~] = B_versus_qr(Bt,phism_tc,qrm,P);

% Compute backiron segment 1 fields
